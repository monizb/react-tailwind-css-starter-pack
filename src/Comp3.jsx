...
const TypeTextStylePrimarySizexsStateDefault = ({
  ...
  isVisible && (
    <section className={`h-6 px-2 py-[3px] bg-violet-600 rounded-md outline outline-1 outline-offset-[-1px] outline-violet-600 inline-flex justify-center items-center gap-1.5 overflow-hidden ${className}`}>
      {showLeftIcon && (
      ...
      {showLabel && (
        <span className={`justify-start text-white text-xs font-medium font-['Inter'] leading-none ${labelClassName}`}>
          {label}
        </span>
      )}
      {showRightIcon && (
