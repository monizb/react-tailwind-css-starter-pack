...
const DynamicButton = (props) => {
  ...
  const buttonStyle = `h-6 px-${px} py-${py} bg-${bgColor} rounded-${rounded} ${
    hasOutline
      ? `outline outline-1 outline-offset-[-1px] outline-${borderColor}`
      : ''
  } inline-flex justify-center items-center gap-1.5 overflow-hidden ${customClasses} ${
    isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
  }`;

  const textStyle = `justify-start text-${textColor} text-${textSize} font-medium font-['${font}'] leading-none`;

  return (
    <button className={buttonStyle} onClick={!isDisabled ? onClick : undefined} disabled={isDisabled}>
      {iconStart && (
        <div data-svg-wrapper className="relative">
          {/* Start Icon */}
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_2041_177)">
              <path
      ...
      {iconEnd && (
        <div data-svg-wrapper className="relative">
          {/* End Icon */}
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_2041_179)">
              <path
