...
const TypeTextStylePrimarySizexs = ({
  ...
  return (
    <button
      ...
      className={`h-6 px-2 py-[3px] bg-violet-600 rounded-md outline outline-1 outline-offset-[-1px] outline-violet-600 inline-flex justify-center items-center gap-1.5 overflow-hidden transition hover:bg-violet-700 active:bg-violet-800 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed ${className}`}
    >
      {showLeftIcon && (
        <span data-svg-wrapper className={`relative ${leftIconClassName}`}>{leftIcon}</span>
      )}
      <span className="justify-start text-white text-[11px] font-medium font-['Inter'] leading-[18px]">
        {title}
      </span>
      {showRightIcon && (
        <span data-svg-wrapper className={`relative ${rightIconClassName}`}>{rightIcon}</span>
      )}
    </button>
  );
};
