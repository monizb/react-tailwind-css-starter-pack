...
const TypeTextStylePrimarySizexs = ({
  ...
  return (
    <button
      type="button"
      className={[
        'h-6 px-2 py-[3px] bg-violet-600 rounded-md inline-flex justify-center items-center gap-1.5 overflow-hidden',
        isOutlined ? 'outline outline-1 outline-offset-[-1px] outline-violet-600' : '',
        isDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-violet-700 active:bg-violet-800 cursor-pointer transition',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
