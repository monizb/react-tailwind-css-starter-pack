...
const TypeTextStylePrimarySizexs = ({
  ...
  return (
    <Component
      className={`h-6 px-2 py-[3px] bg-violet-600 rounded-md outline outline-1 outline-offset-[-1px] outline-violet-600 inline-flex justify-center items-center gap-1.5 overflow-hidden transition-colors
        ${!disabled ? 'hover:bg-violet-700 cursor-pointer' : 'opacity-50 pointer-events-none cursor-not-allowed'}`}
    ...
    >
      {leftIcon && (
        <span data-svg-wrapper className="relative flex-shrink-0">
          {leftIconSVG}
        </span>
      )}
