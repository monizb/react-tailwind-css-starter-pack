...
const TypeTextStylePrimarySizexs = ({
  ...
  const As = as; // Either 'button' or 'div', etc.
  const hasLeftIcon = leftIcon && (leftIconSvg !== undefined || true);
  const hasRightIcon = rightIcon && (rightIconSvg !== undefined || true);
  ...
  return (
    <As
      ...
      className={`h-6 px-2 py-[3px] bg-violet-600 rounded-md outline outline-1 outline-offset-[-1px] outline-violet-600 inline-flex justify-center items-center gap-1.5 overflow-hidden
        ${!disabled ? 'cursor-pointer hover:bg-violet-700 hover:outline-violet-700 transition-colors' : 'opacity-50 cursor-not-allowed'}
        ${className}`}
