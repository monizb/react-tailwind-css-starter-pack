...
const TypeTextStyleWarningSizexsStateHover = ({
  isVisible = true,
  leftIcon = (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clipPath="url(#clip0_2041_529)">
  ...
  isVisible && (
    <section
      className={`h-6 px-2 py-[3px] bg-amber-400 rounded-md inline-flex justify-center items-center gap-1.5 overflow-hidden ${className}`}
      {...containerProps}
    >
      {showLeftIcon && (
        <span data-svg-wrapper className="relative" {...leftIconProps}>
          {leftIcon}
        </span>
      )}
      <span
        className="justify-start text-gray-900 text-[11px] font-medium font-['Inter'] leading-[18px]"
        {...labelProps}
      >
        {label}
      </span>
      {showRightIcon && (
        <span data-svg-wrapper className="relative" {...rightIconProps}>
          {rightIcon}
        </span>
      )}
    </section>
...
export default TypeTextStyleWarningSizexsStateHover;