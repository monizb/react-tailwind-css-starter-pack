...
const DynamicInput = ({
  ...
  hoverableInput = false, // <-- DISABLE HOVER
  focusableInput = true,
  ...
  // Dropdown state for each leadingGroup dropdown
  const [dropdownOpen, setDropdownOpen] = useState(Array(leadingGroup.length).fill(false));
  const [dropdownValues, setDropdownValues] = useState(
    leadingGroup.map(item => (item.type === 'dropdown' ? item.content : null))
  );
                ...
                <div className={item.containerClass} key={idx} style={{ position: 'relative' }}>
                  {item.type === 'dropdown' ? (
                    <>
                      <button
                        type="button"
                        className={item.className + ' flex items-center gap-1 bg-transparent border-none outline-none'}
