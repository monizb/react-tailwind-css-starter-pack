    ...
    <section className="size- inline-flex flex-col justify-start items-start">
      <form className={`${widthClass} flex flex-col justify-start items-start gap-1.5`}>
        <div className="self-stretch flex flex-col justify-start items-start gap-1.5">
          {showLabel && (
            <label className={`justify-start ${labelClass}`}>{label}</label>
          )}
          <div
            className={
              `${widthClass} h-7 ${inputBoxClass}` +
              `${(hoverableInput && inputHovered) ? ' outline-2 outline-blue-500' : ''}` +
              `${(focusableInput && inputFocused) ? ' outline-2 outline-blue-700' : ''}`
            }
            tabIndex={-1}
            onMouseEnter={() => hoverableInput && setInputHovered(true)}
            onMouseLeave={() => hoverableInput && setInputHovered(false)}
          >
            <nav className="size- flex justify-start items-center">
              {leadingGroup.map((item, idx) => (
                <div className={item.containerClass} key={idx}>
                  <span className={item.className}>{item.content}</span>
                  {item.type === 'dropdown' && (
                    <span className="relative ml-1">{item.icon}</span>
                  )}
                </div>
              ))}
            </nav>
            <main className="flex-1 h-7 pl-2 flex justify-start items-center gap-2">
              ...
              <input
                type="text"
                className={inputClass + ' bg-transparent p-0 border-0 outline-none w-full'}
                value={value}
                ...
                onFocus={() => focusableInput && setInputFocused(true)}
                onBlur={() => focusableInput && setInputFocused(false)}
                style={{ fontFamily: 'Inter' }}
              />
            </main>
            <aside className="size- flex justify-start items-center">
        ...
        {showHint && (
          <footer className={`self-stretch justify-start ${hintClass}`}>
            {hintText}
          </footer>
        )}
      </form>
